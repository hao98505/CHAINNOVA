...
interface RelayerState {
  processedSolTransferIds: string[];
...
async function relaySolanaToEvm(
  ...
  if (state.processedSolTransferIds.includes(event.transferId)) return;
  ...
  try {
    ...
    state.processedSolTransferIds.push(event.transferId);
    saveState(state);
  } catch (err: any) {
...
async function watchSolanaBridgeOut(
  ...
  async function poll() {
    try {
      ...
      while (true) {
        const opts: any = { limit: 50, commitment: "finalized" };
        ...
        const batch: ConfirmedSignatureInfo[] = await connection.getSignaturesForAddress(
          programId,
          opts,
          "finalized"
        );
      ...
      const processedSet = new Set(state.processedSolTransferIds);
      ...
      state.lastSolSignature = allSigs[0].signature;
      saveState(state);
    } catch (err: any) {
...
async function relayEvmToSolana(
  ...
  console.log(`  [queued] EVM->Solana complete_transfer not yet wired (needs Anchor client + keypair)`);
  ...
  console.log(`  Transfer NOT marked processed — will retry on next poll`);
}
  ...
  function schedulePoll() {
    setTimeout(() => {
      (async () => {
        ...
        const safeLatest = latest - BigInt(12);
        if (from > safeLatest) return;
        const toBlock = from + BigInt(1000) < safeLatest ? from + BigInt(1000) : safeLatest;
...
async function main() {
  ...
  console.log(`Processed: ${state.processedSolTransferIds.length} SOL->EVM, ${state.processedEvmTransferIds.length} EVM->SOL`);
  console.log();
